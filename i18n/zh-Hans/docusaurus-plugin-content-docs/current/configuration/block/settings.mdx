...
决定方块在受到活塞作用时的反应，默认值为 `PUSH_PULL`。名称与 Minecraft 26.3 一致，在旧版服务端上会映射到对应的原版行为。已有配置仍可使用旧名称。
...
`PUSH_PULL` 可推可拉，`PUSH` 只可推动，`POPPED` 被推动时破坏，`IMMOVEABLE` 不可移动。`IGNORE_ENTITY` 对应原版此前的 `IGNORE` 行为。
