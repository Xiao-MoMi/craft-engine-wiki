...
`/ce reload config` updates configuration and workflow definitions, but does not rebuild the pack. `/ce reload all` runs workflows whose `trigger` includes `reload_pack`; the default `development` workflow handles generation, upload, and sending. If your build workflow has no such trigger, run `/ce workflow [name]` explicitly. Sending updates requires a `send_pack` step and only targets players currently using that pack.
