...
Self hosting serves ZIP files through CraftEngine's built-in HTTP server. Downloads use your server's outbound bandwidth. To configure resource pack generation, uploads, and sending, see [resource pack workflows](../workflows).
...
resource-pack:
  packs:
    development:
      type: self
      default: true
      storage_path: ./cache/hosted/development/resource_pack.zip
    production:
      type: self
      default: false
      storage_path: ./cache/hosted/production/resource_pack.zip
...
Add this workflow to the same `resource-pack` section. Run `/ce reload host` after changing host settings, or `/ce reload` after only changing the workflow. Once the ZIP exists, run `/ce workflow upload_development`.
