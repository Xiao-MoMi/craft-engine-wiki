...
```text
/pack enable production
/pack disable development
/pack reset production
```
...
Sends one summary after completion: names a single target, or reports the number of players whose preferences changed for multiple targets. Unchanged preferences and failures are also reported. Add `--silent` (or `-s`) to suppress command feedback.
