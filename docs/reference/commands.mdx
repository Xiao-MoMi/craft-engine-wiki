...
Execute a named workflow from `resource-pack.workflows`, such as `/ce workflow production`. Tab completion lists loaded workflows. Default permission: `ce.command.admin.pack_workflow`.
...
The former `/ce upload` command has been replaced by workflow `upload` steps. Uploading alone does not resend packs: add a `send_pack` step to send updates to online players currently using the specified pack. The step's `pack` value must match an ID under `resource-pack.packs`.

### pack

`/ce pack [enable/disable/reset] [pack-id] (player)`

Save a player's resource pack preference. `pack-id` refers to an entry under `resource-pack.packs`.
...
Omit `player` to change your own preference. When supplied, it must be an exact online player name or a UUID; use a UUID for an offline player. The console must specify a target. Player senders need `ce.resourcepack.<pack-id>`; the command's default permission in `commands.yml` is empty.
...
```text
/ce pack enable production
/ce pack disable development
/ce pack reset production Steve
```
