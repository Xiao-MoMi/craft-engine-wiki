...
Determines how the block reacts to pistons. Default: `PUSH_PULL`. Names follow Minecraft 26.3 and are mapped to the corresponding native behavior on older servers. Legacy names remain accepted for existing configurations.
...
`PUSH_PULL` allows pushing and pulling, `PUSH` allows pushing only, `POPPED` breaks when pushed, and `IMMOVEABLE` prevents movement. `IGNORE_ENTITY` corresponds to vanilla's former `IGNORE` reaction.
